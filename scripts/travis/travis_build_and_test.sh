...
if $WITH_CMAKE; then
  ...
  if $WITH_IO; then
    IO_ARGS="-DUSE_HDF5=ON -DUSE_OPENCV=ON -DUSE_LMDB=ON -DUSE_LEVELDB=ON -DUSE_SNAPPY=ON"
  else
    IO_ARGS="-DUSE_HDF5=OFF -DUSE_OPENCV=OFF -DUSE_LMDB=OFF -DUSE_LEVELDB=OFF -DUSE_SNAPPY=OFF"
  fi
